...
class  CampaignController extends Controller
{
    ...
    public function index(ListRequest $request): View
    {
        ...
        $cacheKey = "campaigns_index_{$startDate->format('Ymd')}_{$endDate->format('Ymd')}_{$orderBy}_{$sortDirection}_{$request->page}_{$request->brand}";
        $campaigns = Cache::remember($cacheKey, 600, function() use ($startDate, $endDate, $request, $orderBy, $sortDirection) {
            return Campaign::select('campaigns.name', 'brand_id')
                ...
                ->whereBetween('campaigns.created_at', [$startDate, $endDate])
